...
void BrainBWin::updateHeroes ( const QImage &image, const int &x, const int &y )
{
        ...
        if ( start && !brainBThread->get_paused() ) {
                //kiszámoljuk az egér és a Samu Entropy doboz közötti távolságot, majed ennek eredményét a dist változóba mentjük el.
                int dist = ( this->mouse_x - x ) * ( this->mouse_x - x ) + ( this->mouse_y - y ) * ( this->mouse_y - y );
                //Ha a távolság nagyobb mint 121 pixel, megnöveljük a nofLost értékét 1-el.
                if ( dist > 121 ) {
                        ++nofLost;
                        nofFound = 0;
                        
                        if ( nofLost > 12 ) {
                                //Ha a játékos jól szabályozta a karaktereket az előző időintervallumokban???
                                // akkor a feltétel igaz lesz és a user elvesztette a Samu Entrophy karaktert
                                // A kép vizuális komplexitása elmentésre kerül
